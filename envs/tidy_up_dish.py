...
class TidyUpDish(BaseEnv):
    ...
    def _add_workspace(self):
        ...
        # add sink
        builder = self._scene.create_actor_builder()
        # builder.add_visual_from_file(scale=[1.5,2.0,1.4], filename='./assets/kitchen/sink/stainless_steel_sink.dae')
    ...
    def _add_actor(self):
        ...
        teapot = builder.build(name = 'teapot') # can not be affected by external forces
        teapot.set_pose(sapien.Pose(p=[-0.753, -0.973, 0.153],q=[-0.514,-0.504,-0.503,-0.478]))
        ...
        builder.add_box_collision(pose =sapien.Pose(p=[0.633,0.0, 0.0]), half_size=[0.1, 0.01, 0.1])
        ...
        fruits = builder.build(name = 'fruits') # can not be affected by external forces
        ...
        # banana.set_pose(sapien.Pose(p=[-0.831, 0.782, 0.114],q=[0.627, -0.307, 0.675, -0.237])) # pick on the table
