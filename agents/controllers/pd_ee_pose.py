...
class PDEEPoseController(BaseController):
    def __init__(self, ee_link_name='xarm_gripper_base_link', **kwargs):
        """
    ...
    def ee_pose_at_base(self):
        to_base = [link for link in self.robot.get_links() if link.get_name() == 'link_base'][0].get_pose().inv()
    ...
    def compute_target_pose(self, action, frame='ee'):
        ...
        if self.config['use_delta']:
            delta_pos, delta_rot = action[3:6], action[6:9]
            ...
            # NOTE(haoyang): can be used for the mobile base?
            # maybe we still need to define the trajactory seperately
            target_pose = self.pose_at_base(sapien.Pose(action[0:3], action[3:7])) # Transfer target pose to base coordinate.
            # target_pos, target_rot = action[3:6], action[6:9]
